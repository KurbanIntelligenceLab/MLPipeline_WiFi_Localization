...
base_path = file.path(dirname(path), "RegressionPipeline")
print(base_path)
...
input_file_name_test = args[4]
model_time_file_name = args[5]


# already_running = "no"
# result_dir_name = "wireless_pred"
# input_file_name = "train_data.csv"
# input_file_name_test = "test_data.csv"
...
out <- load_data(file.path(base_path, "data", input_file_name), file.path(base_path, "data", input_file_name_test))
train_data <- out[[1]]
test_data <- out[[2]]
